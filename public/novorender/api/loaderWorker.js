...
var BufferReader = class {
	...
	_f64;
	read(ar, size) {
		if (size == 0) return ar.subarray(0, 0);
...
var {
	...
	OptionalVertexAttribute: OptionalVertexAttribute2,
	PrimitiveType: PrimitiveType2,
	TextureSemantic: TextureSemantic2,
} = __exports;
...
function getVertexAttribNames(optionalAttributes, deviations, hasMaterials, hasObjectIds) {
	...
	if (optionalAttributes & OptionalVertexAttribute2.normal) attribNames.push('normal');
	if (optionalAttributes & OptionalVertexAttribute2.texCoord) attribNames.push('texCoord');
	if (optionalAttributes & OptionalVertexAttribute2.color) attribNames.push('color');
	if (optionalAttributes & OptionalVertexAttribute2.projectedPos) attribNames.push('projectedPos');
...
function getChildren(parentId, schema, separatePositionBuffer, predicate) {
	...
	for (let i = 0; i < childInfo.length; i++) {
		...
		const { primitives, gpuBytes } = aggregateSubMeshProjections(
			schema.subMeshProjection,
			subMeshProjectionRange,
			separatePositionBuffer,
			predicate
		);
...
function* getSubMeshes(schema, predicate) {
	...
	for (let i = 0; i < subMesh.length; i++) {
		...
		if (predicate?.(objectId) ?? true) {
			...
			const materialType =
				materialIndex == 255 &&
				subMesh.textures.count[i] == 0 &&
				(primitive == PrimitiveType2.triangle_strip || primitive == PrimitiveType2.triangles)
					? MaterialType2.elevation
...
function getGeometry(schema, separatePositionBuffer, enableOutlines, highlights, predicate) {
	...
	for (const { materialType, primitiveType, attributes, deviations, subMeshIndices } of groups.values()) {
		...
		if (subMeshIndices.length == 0) continue;
		...
		const vertexAttributes = {
			position: {
				...
				componentType: 'SHORT',
				normalized: true,
				byteOffset: attribOffsets['position'],
				byteStride: separatePositionBuffer ? 0 : stride,
			},
			normal:
				(attributes & OptionalVertexAttribute2.normal) != 0
					? {
							kind: 'FLOAT_VEC3',
							buffer: bufIdx.primary,
							componentCount: 3,
							componentType: 'BYTE',
							normalized: true,
							byteOffset: attribOffsets['normal'],
							byteStride: stride,
					  }
					: null,
			material: hasMaterials
				? {
						kind: 'UNSIGNED_INT',
						buffer: bufIdx.primary,
						componentCount: 1,
						...
						normalized: false,
						byteOffset: attribOffsets['materialIndex'],
						byteStride: stride,
				  }
				: null,
			objectId: hasObjectIds
				? {
						kind: 'UNSIGNED_INT',
						buffer: bufIdx.primary,
						componentCount: 1,
						...
						normalized: false,
						byteOffset: attribOffsets['objectId'],
						byteStride: stride,
				  }
				: null,
			texCoord:
				(attributes & OptionalVertexAttribute2.texCoord) != 0
					? {
							kind: 'FLOAT_VEC2',
							buffer: bufIdx.primary,
							componentCount: 2,
							componentType: 'HALF_FLOAT',
							normalized: false,
							byteOffset: attribOffsets['texCoord'],
							byteStride: stride,
					  }
					: null,
			color:
				(attributes & OptionalVertexAttribute2.color) != 0
					? {
							kind: 'FLOAT_VEC4',
							buffer: bufIdx.primary,
							componentCount: 4,
							componentType: 'UNSIGNED_BYTE',
							normalized: true,
							byteOffset: attribOffsets['color'],
							byteStride: stride,
					  }
					: null,
			projectedPos:
				(attributes & OptionalVertexAttribute2.projectedPos) != 0
					? {
							kind: 'FLOAT_VEC4',
							buffer: bufIdx.primary,
							componentCount: 3,
							componentType: 'SHORT',
							normalized: true,
							byteOffset: attribOffsets['projectedPos'],
							byteStride: stride,
					  }
					: null,
			deviations:
				deviations != 0
					? {
							kind: deviationsKind,
							buffer: bufIdx.primary,
							componentCount: deviations,
							componentType: 'HALF_FLOAT',
							normalized: false,
							byteOffset: attribOffsets['deviations'],
							byteStride: stride,
					  }
					: null,
			triangles0: trianglePosBuffer
				? {
						kind: 'FLOAT_VEC4',
						buffer: bufIdx.triPos,
						componentCount: 3,
						componentType: 'SHORT',
						normalized: true,
						byteOffset: 0,
			...
			triangles2: trianglePosBuffer
				...
				: null,
			trianglesObjId: trianglePosBuffer
				? {
						kind: 'UNSIGNED_INT',
						buffer: bufIdx.triId,
						componentCount: 1,
...
function parseNode(id, separatePositionBuffer, enableOutlines, version3, buffer, highlights, applyFilter) {
...
var LoaderHandler = class {
	...
	parseBuffer(buffer, params) {
		...
		const { childInfos, geometry } = parseNode(id, separatePositionsBuffer, enableOutlines, version3, buffer, highlights, applyFilter);
